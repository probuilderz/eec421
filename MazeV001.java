...
public class MazeV001 extends java.applet.Applet implements Runnable {
...
//         }
         ...
         if (color[visitedCode] == null)
            color[visitedCode] = Color.white;// color[emptyCode];
    ...
    void checkSize() {
       ...
       if (getWidth() != width || getHeight() != height) {
          width  = getWidth();
          height = getHeight();
          int w = (width - 2*border) / getColumns();  //columns
          int h = (height - 2*border) / getRows();         // rows
          left = (width - w*getColumns()) / 2;
          top = (height - h*getRows()) / 2;
          totalWidth = w*getColumns();
          totalHeight = h*getRows(); 
       }
    }
    ...
    synchronized void redrawMaze(Graphics g) {
        ...
        if (mazeExists) {
           int w = totalWidth / getColumns();  // width of each cell
           int h = totalHeight / getRows();    // height of each cell
    ...
    public void run() {
	   ...
	   if (keep.equals("y")|| keep.equals("Y")){
		   ans = true;
		   System.out.print("Provide the dimension of the Maze Applet D x D: ");
		   length_select = scan.nextInt();
		   setRows(length_select);
//		   System.out.print("Provide the columns of the Applet: ");
		   setColumns(length_select);
	   }else{
		   ans = false;
		   System.exit(0);
	   }
...
//          
    ...
    void makeMaze() {
        ...
        if (maze == null)
           maze = new int[length_select][length_select];
        ...
        int[] wallrow = new int[(getRows()*getColumns())/2];  // position of walls between rooms
        int[] wallcol = new int[(getRows()*getColumns())/2];
        for (i = 0; i<length_select; i++)  // was "rows" --start with everything being a wall
            for (j = 0; j < length_select; j++)
                maze[i][j] = wallCode;
        for (i = 1; i<getRows()-1; i += 2)  // make a grid of empty rooms
            for (j = 1; j<getColumns()-1; j += 2) {
                emptyCt++;
                maze[i][j] = -emptyCt;  // each room is represented by a different negative number
                if (i < getRows()-2) {  // record info about wall below this room
                    wallrow[wallCt] = i+1;
                    wallcol[wallCt] = j;
                    wallCt++;
                }
                if (j < getColumns()-2) {  // record info about wall to right of this room
                    wallrow[wallCt] = i;
                    wallcol[wallCt] = j+1;
                    wallCt++;
                }
        ...
        for (i=1; i<getRows()-1; i++)  // replace negative values in maze[][] with emptyCode
